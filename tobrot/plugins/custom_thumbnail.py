...
async def save_thumb_nail(client, message):
    ...
    if message.reply_to_message is not None:
        ...
        await ismgs.edit(
            "✅ Custom video / file thumbnail saved BY @KOT_BOTS. "
            + "This image will be used in the upload, till /clearthumbnail."
        )
    else:
        await ismgs.edit("❌ Reply to a photo to save custom thumbnail by @KOT_BOTS")
...
async def clear_thumb_nail(client, message):
    ...
    if os.path.exists(thumb_image_path):
        ...
        await ismgs.edit("✅ Custom thumbnail cleared successfully by @kot_bots.")
    else:
        await ismgs.edit("❌ Nothing to clear by @kot_bots.")
